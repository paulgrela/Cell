...
void CellEngineOpenGLVisualiserOfFullAtomSimulationSpace::RenderSpace1(UnsignedInt& NumberOfAllRenderedAtoms, const vmath::mat4& ViewMatrix, uint32_t AtomOffsetTotal, vector<GPUParticle>& GPUParticles, vector<GPUAtom>& GPUAtoms, vector<GPUAtomLocal>& GPUAtomsLocal)
{
    try
    {
...
void CellEngineOpenGLVisualiserOfFullAtomSimulationSpace::RenderSpace2(UnsignedInt& NumberOfAllRenderedAtoms, const vmath::mat4& ViewMatrix, uint32_t AtomOffsetTotal, vector<GPUParticle>& GPUParticles, vector<GPUAtom>& GPUAtoms)
{
    try
    {
        const auto start_time111 = chrono::high_resolution_clock::now();
        ...
        FOR_EACH_SECTOR_IN_XYZ_ONLY
            if (CellEngineDataFileObjectPointer->GetParticles()[ParticleSectorXIndex][ParticleSectorYIndex][ParticleSectorZIndex].Particles.empty() == false)
                if (RenderObject(CellEngineDataFileObjectPointer->GetParticles()[ParticleSectorXIndex][ParticleSectorYIndex][ParticleSectorZIndex].Particles.begin()->second.ListOfAtoms.back(), Particle(), ViewMatrix, true, false, true, NumberOfAllRenderedAtoms, false, !CellEngineConfigDataObject.ShowDetailsInAtomScale) == true)
                    if (CellEngineConfigDataObject.ShowDetailsInAtomScale == true)
                        for (const auto& ParticleObject : CellEngineDataFileObjectPointer->GetParticles()[ParticleSectorXIndex][ParticleSectorYIndex][ParticleSectorZIndex].Particles)
                        {
                            GPUParticle GPUParticleObject;
...
void CellEngineOpenGLVisualiserOfFullAtomSimulationSpace::RenderSpace3(UnsignedInt& NumberOfAllRenderedAtoms, const vmath::mat4& ViewMatrix, uint32_t AtomOffsetTotal, uint32_t ParticlesOffsetTotal, vector<GPUParticle>& GPUParticles, vector<GPUAtom>& GPUAtoms)
{
    try
    {
        const auto start_time114 = chrono::high_resolution_clock::now();
        ...
        FOR_EACH_SECTOR_IN_XYZ_ONLY
        {
            ...
            if (CellEngineDataFileObjectPointer->GetParticles()[ParticleSectorXIndex][ParticleSectorYIndex][ParticleSectorZIndex].Particles.empty() == false)
                if (RenderObject(CellEngineDataFileObjectPointer->GetParticles()[ParticleSectorXIndex][ParticleSectorYIndex][ParticleSectorZIndex].Particles.begin()->second.ListOfAtoms.back(), Particle(), ViewMatrix, true, false, true, NumberOfAllRenderedAtoms, false, !CellEngineConfigDataObject.ShowDetailsInAtomScale))
                    if (CellEngineConfigDataObject.ShowDetailsInAtomScale == true)
                    {
...
void CellEngineOpenGLVisualiserOfFullAtomSimulationSpace::RenderSpace(UnsignedInt& NumberOfAllRenderedAtoms, UnsignedInt& NumberOfFoundParticlesCenterToBeRenderedInAtomDetails, const vmath::mat4& ViewMatrix)
{
    try
    {
        ...
        RenderSpace1(NumberOfAllRenderedAtoms, ViewMatrix, AtomOffsetTotal, GPUParticles, GPUAtoms, GPUAtomsLocal);
        ...
        ExecutionDurationTimeForCopyingParticlesToGraphicMemory += chrono::duration(stop_time1 - start_time1);
                            ...
                            if (CellEngineConfigDataObject.NumberOfStencilBufferLoops > 1)
                            {
                                LoggersManagerObject.Log(STREAM("MOUSE0 X = " << MousePositionLocal.s.X << " " << MousePositionLocal.s.Y << " " << Info.WindowHeight));
                            ...
                            if (PressedRightMouseButton != 1)
                                DrawChosenAtomUsingStencilBuffer1(ViewMatrix, ClickedObjectID, NumberOfAllRenderedAtoms, GPUAtomsLocal);
    }
    CATCH("rendering full atom simulation space");
}

inline void CellEngineOpenGLVisualiserOfFullAtomSimulationSpace::DrawChosenAtomUsingStencilBuffer1(const vmath::mat4& ViewMatrix, const GLuint PartOfStencilBufferIndex, UnsignedInt& NumberOfAllRenderedAtoms, const vector<GPUAtomLocal>& GPUAtomsLocal)
{
    try
    {
        if (CellEngineConfigDataObject.NumberOfStencilBufferLoops > 1)
