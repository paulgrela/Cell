...
void CellEngineOpenGLVisualiser::DiagnoseFrameBufferObject() const
{
    try
    {
        glBindFramebuffer(GL_FRAMEBUFFER, FrameBufferObject);
    ...
    CATCH("diagnosing frame buffer object")
}
...
void CellEngineOpenGLVisualiser::LoadShadersPhong()
{
    try
    {
        ...
        Uniforms.SpecularPower = glGetUniformLocation(ParticlesAtomsTrianglesShadersProgram, "specular_power");
    }
...
void CellEngineOpenGLVisualiser::DrawAllFoundBondsBetweenAtoms(const vector<float>& LinesVertexesLocal, const vmath::mat4& ViewMatrix) const
{
    try
    {
        ...
        glUseProgram(ParticlesAtomsBondsLinesShadersProgram);

        constexpr int UseMainLineColor = 1;
        glUniform1i(glGetUniformLocation(ParticlesAtomsBondsLinesShadersProgram, "UseMainLineColor"), UseMainLineColor);

        const auto MainLineColor = CellEngineConfigDataObject.MainColorForAllBondsBetweenAtoms;
        glUniform3fv(glGetUniformLocation(ParticlesAtomsBondsLinesShadersProgram, "MainLineColor"), 1, MainLineColor);

        vmath::mat4 MVP = ProjectionMatrixGlobal * ViewMatrix;
        glUniformMatrix4fv(glGetUniformLocation(ParticlesAtomsBondsLinesShadersProgram, "MVP"), 1, GL_FALSE, MVP);
...
void CellEngineOpenGLVisualiser::FindAllBondsToDrawForParticle(const Particle& ParticleObject, vector<pair<UnsignedInt, UnsignedInt>>& BondsToDraw, const bool DrawBonds, const vmath::mat4& ViewMatrix)
{
    try
    {
        if (DrawBonds == true)
        ...
        if (CellEngineConfigDataObject.ShowAtomsInEachPartOfTheCellWhenObserverIsFromOutside == false)
        {
            if (ZNew > CellEngineConfigDataObject.CutZ)
            {
                ...
                if (TakeObjectAsVisible == true)
                {
                    ...
                    const float minDist = CellEngineConfigDataObject.Distance - 450;
                    const float maxDist = CellEngineConfigDataObject.Distance + 450;
...
inline bool CellEngineOpenGLVisualiser::CreateUniformBlockForVertexShader(const vmath::vec3& Position, const vmath::vec3& Color, const vmath::mat4& ViewMatrix, vmath::mat4& ModelMatrix, const bool CountNewPosition, const bool DrawCenter, const bool DrawOutsideBorder, bool DrawAdditional) const
{
    ...
    try
    {
        ...
        {
            FinalVisibilityInModelWorld = GetFinalVisibilityInModelWorld(Position, &UniformsBlockObject, CountNewPosition, DrawOutsideBorder);
            if (DrawCenter == true)
                DrawCenterPoint(&UniformsBlockObject, ModelMatrix);
        }
...
inline void CellEngineOpenGLVisualiser::PrepareOpenGLToRenderObjectsOnScene() const
{
    try
    {
        static constexpr GLfloat gray[] = { 0.1f, 0.1f, 0.1f, 0.0f };
        static constexpr GLfloat ones[] = { 1.0f };
...
void CellEngineOpenGLVisualiser::ComputeInShaderCopyParticlesAndAtomsDataToGPUMemoryAndRender(const vmath::mat4& ViewMatrix)
{
    try
    {
...
void CellEngineOpenGLVisualiser::OnKey(const int Key, const int Action)
{
    try
    {
        if (Action)
...
void CellEngineOpenGLVisualiser::OnMouseWheel(const int Pos)
{
    try
    {
        if (CellEngineConfigDataObject.ViewPositionZ >= CellEngineConfigDataObject.Distance - BackSize)
...
void CellEngineOpenGLVisualiser::OnMouseButton(const int Button, int Action)
{
    try
    {
        if (Button == 0)
...
void CellEngineOpenGLVisualiser::OnMouseMove(const int X, const int Y)
{
    try
    {
...
void CellEngineOpenGLVisualiser::OnResize(const int Width, const int Height)
{
    try
    {
