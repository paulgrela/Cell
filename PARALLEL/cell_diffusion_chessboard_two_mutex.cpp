...
//
// Each wall carries TWO mutexes, as in producer_consumer_two_mutex.cpp:
//   m1 + cvProposal guard the proposals channel  (sender -> judge)
//   m2 + cvVerdict  guard the verdicts channel   (judge  -> sender)
// The two non-blocking handoff sections take both together with
// std::lock(lk1, lk2). The two wait() sections take exactly ONE mutex --
// the one their condition variable is paired with. That split is the
// whole point: wait() releases only the lock you hand it, so holding the
// other one across the sleep would lock the peer out of the channel it
// needs to wake you.
//
// ---------------------------------------------------------------- chessboard
//
...
struct Wall 
{
    std::mutex m1;                            // guards the proposals channel
    std::mutex m2;                            // guards the verdicts channel
    std::condition_variable cvProposal;       // paired with m1
    std::condition_variable cvVerdict;        // paired with m2
...
void sendHalfStep(Cube& c) 
{
    ...
    for (int d = 0; d < 6; ++d) 
	{
        ...
        {   
			// handoff, no wait() here -- safe to take both mutexes together
            std::unique_lock<std::mutex> lk1(W.m1, std::defer_lock);
            std::unique_lock<std::mutex> lk2(W.m2, std::defer_lock);
            std::lock(lk1, lk2);
            W.proposals[slot] = c.outParticles[d];
            W.proposalsReady[slot] = true;
        }
		
        W.cvProposal.notify_all();
    }

    // only now block, collecting the six verdicts.
    // wait() takes ONLY m2 -- the mutex its condition variable is paired with.
    // Holding m1 across this wait would freeze the judge out of the proposals
    // channel for as long as we sleep, which is the deadlock from before.
    for (int d = 0; d < 6; ++d) 
	{
        if (c.wall[d] < 0) 
		{ 
        ...
        Wall& W = *walls[c.wall[d]];
        std::unique_lock<std::mutex> lk2(W.m2);
        W.cvVerdict.wait(lk2, [&] {  return W.verdictsReady[slot]; });
...
void judgeHalfStep(Cube& c) 
{
    ...
    for (int d = 0; d < 6; ++d) 
	{
        ...
        // wait() takes ONLY m1 -- the mutex its condition variable is paired with
        std::vector<Particle> incoming;
        {
            std::unique_lock<std::mutex> lk1(W.m1);
            W.cvProposal.wait(lk1, [&] { return W.proposalsReady[slot]; });
            incoming = std::move(W.proposals[slot]);
